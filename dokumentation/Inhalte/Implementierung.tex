...
Wenn ein Spieler eine Aktion durchführen möchte, verschickt er in der Offline-Variante kein Paket an den spe\_ed-Server,
sondern ruft am \Code{GameService} die Methode \Code{do_action(player: Player, action: Action)} auf.
Die Methode manipuliert dabei das \Code{Game}-Objekt, sodass dies eine äquivalente Aktualisierung zum erhalten des
Spiels im JSON-Format durch den spe\_ed-Server ist.
Der grobe Ablauf dieser Methode ist, dass bei dem \Code{Turn}-Objekt die Methode \Code{action(player: Player)}
aufgerufen wird, um zu prüfen ob der Spieler eine Aktion machen darf und ob ein neuer Spielzug nach dieser Aktion
beginnt.
Nachfolgend wird dann die Aktion mit der Methode \Code{get_and_visit_cells(player: Player, action: Action)} durchgeführt
und das \Code{Game} aktualisiert.
Dabei wird der Spieler aktualisiert (x- und y-Koordinate, speed), der die Ausführung der Aktion eingeleitet hat.
Das Spiel wird zunächst nicht verändert, sondern erst, wenn alle Spieler ihre Aktionen durchgeführt haben und ein neuer
Spielzug durch das \Code{Turn}-Objekt eingeleitet wurde.
Dies hilft dabei Kollisionen der Spieler besser zu erkennen.
Sollte also ein neuer Spielzug durch die Aktion entstanden sind, werden die Spieler in die \Code{Cell}s eingetragen,
Spieler mit Kollisionen inaktiv geschaltet und geprüft ob das Spiel beendet ist.
Dieser Ablauf lässt sich genauer im Diagramm nachvollziehen. \todo{Diagramm einfügen} \\
...
zu
prüfen, ob Aktionen zum Verlieren führen.
