...
Mit der Methode \Code{creat\_next\_action()} soll die Funktionalität implementiert werden, als menschlicher Spieler eine
Aktion durchführen zu können.

Die dritte Methode \Code{end()} ist dafür gedacht eine letzte Ausgabe nach dem Ende des Spiels auszugeben.
...
Dadurch war es leicht, strukturierte Tabellen auf die Konsole auszugeben, was in der Methode \Code{update(game: Game)}
geschieht.
...
Zur Implemenierung der Methode \Code{creat\_next\_action()} wurde die Eingabe der Konsole durch die Methode \Code{input}
...
Die grafische Oberfläche wurde in der Klasse \Code{GraphicalView} umngesetzt.
...
Bei der Implementierung der Methode \Code{create\_next\_action()} für die Eingabe der menschlichen Spieler haben wir die
