...
Um sicherzustellen, dass Software erwartungsgemäß funktioniert und eine Weiterentwicklung vereinfacht wird, macht es
Sinn, verschiedene Aspekte zu berücksichten, die für eine verbesserte Qualität der Software sorgen.
...
Beim Aufbau der Software haben wir uns für ein dreischichtige Architektur und den Einsatz des \ac{MVC}-Patterns
entschieden.
Die Architektur ist in \Abbildung{fig:schichtenarchitektur} dargestellt und beinhaltet neben der Schicht zur Anzeige
des Spiels in einer Oberfläche die Logik-Schicht, in der \ua die implementiert wurde, wie ein Spiel abläuft, wie
die Daten zur Kommnuikation mit dem Server übersetzt werden können und auch wie die \ac{KI}s funktionieren sollen.
...
Diese dienen während der Implementierung dazu, ein gewüncshtes Szenario abzubilden und anschließend den Code so zu
implementieren, dass der Test erfolgreich durchläuft.
Anschließend ist eine Refaktorisierung möglich, \dasheisst der Code wird verändert, die Funktionalität sol laber gleich
bleiben.
...
durch eine Änderung keine ungewollten Nebeneffekte eintreten, sondern der Entwickler sicher sein kann, dass bei grünen
...
Um sicherzustellen, dass unabhängig vom Autor der Code einheitlich und wartbar aufgebaut ist, wurde auf die Einhaltung
...
werden.
Dabei war Pylint als Tool hilfreich, dass sich als Plugin direkt in PyCharm-IDE von JetBrains integrieren ließ und somit
bereits bei der Arbeit am Code unmittelbar Verbesserungsvorschläge machen konnte.
...
Dazu zählt \bspw, dass wir an Methoden bei den Parametern und dem Rückgabewert jeweils eine Typangabe ergänzt, obwohl
dies in Python nur optional ist.
