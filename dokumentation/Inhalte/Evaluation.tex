...
Im zweiten Teil sollte somit sichergestellt werden, dass nur noch starke \ac{KI}s gegeneinander antreten und somit eine
bessere Vergleichbarkeit der Siegquoten möglich ist. \\

Am Ende von 500 simulierten Spielen mit \todo{XX} berechneten Spielzügen wurde erneut die SQL-Abfrage aus
...
Das Ergebnis wird in \Tabelle{tab:evaluation-ki-konfiguration-zwei} gezeigt.
Es zeigt, dass eine Kombination aus der \Code{PathfindingAI} und der \Code{SearchTreeAI} \todo{XX} im Vergleich zu unseren anderen
Lösungsansätzen die besten Ergebnisse in den Spielen erzielt, sodass diese schlussendlich auch für die Abgabe des
Projektes ausgewählt wurde.
