...
Die \Code{PathfindingSearchTreeAI} soll die Stärken der \Code{SearchTreeAI} und der \Code{PathfindingAI}
kombinieren.
Zudem wird die Entscheidung der \Code{PathfindingAI} priorisiert.
...
Die Priorisierung hinsichtlich der \Code{PathfindingAI} ist so zu verstehen, dass der \Code{PathfindingSearchTreeAI} bei
der Initialisierung ein Parameter übergeben wird, der eine Toleranz (zwischen 0 und 1) darstellt.
Eine mögliche Aktion der \Code{SearchTreeAI} muss mindestens, die Anzahl Pfade multipliziert mit der Toleranz, erreichen
können, wie die beste Aktion der \Code{PathfindingAI}.

\begin{equation}
\label{eq:toleranz-pathfinding-searchtree-ai}
Erreichbare\_Pfade * Toleranz >= Maximal\_erreichbare_Pfade
\end{equation}
...
zwingen lässt, jedoch mit dem Risiko eine vermeidbare Kollision zu verursachen.
