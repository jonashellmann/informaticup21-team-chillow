...
Dabei wird jedoch nicht die Idee verfolgt, die anderen Mitspieler dazu zu bringen kürzer zu überleben als man selber.
Nachfolgend werden derartige Lösungsideen beschrieben und erklärt, warum wir uns gegen die Implementierung entschieden
haben. \\

Ein möglicher Lösungsansatz stand zur Diskussion, die gegnerischen Spieler an den Spielfeldrand zu drängen und ihn somit
zum Verlieren zu zwingen.
Bei diesem Ansatz muss die eigene KI dicht an die gegnerischen \ac{KI} fahren, damit diese zu Entscheidungen gezwungen
wird, die nicht optimal sind und ein vorzeitiges ausscheiden bedeuten.
...
einen Gegner zu umkreisen.
Dadurch würde man dazu beitragen, dass der Gegner entweder nur noch wenige Züge zur Verfügung hat und den verbleibenden
Platz nutzt, oder einen sechsten Zug mit einer Geschwindigkeit größer 3 nutzen muss, um aus dem Feld wieder heraus zu
kommen.
Die Berechnung mit einem sechsten Zug über die umkreisenden Spur zu kommen ist nicht trivial und zudem existiert auch
teilweise keine Option, dies zu schaffen.
...
Um einen gegnerischen Spieler umkreisen zu können, muss die aggressive \ac{KI} in den meisten Fällen schneller sein als
das Ziel.
Dies bedeutet aber auch, dass Geschwindigkeiten erreicht werden müssen, die Löcher in der Spur in jedem sechsten Zug
hinterlassen, wodurch das umkreisen dann kaum Wirkung hätte.
Außerdem hat sich ein hohe Geschwindigkeit bei der Evaluation unserer anderen implementierten Lösungsansätze bereits als
riskant herausgestellt.
Die erhöhte Geschwindigkeit sorgt dafür, dass man häufig nur wenige oder keine Aktion zur Auswahl hat, die im
nächsten Spielzug nicht zum Verlieren führen.
Daher steigt die Warscheinlichkeit, dass die eigene \ac{KI} durch ihr agressives Verhalten frühzeitig ausscheidet.
...
Aufgrund dieser Problematik haben wir uns dagegen entscheiden eine \ac{KI} zu implementieren, die proaktiv für das
Ausscheiden anderer Spieler sorgt, da das eigene Ausscheiden eine zu präsente Gefahr darstellt.
